...
string mySQLVersion = db.GetString("select concat('MySQL version : ', version())");@@

@@Database db = new Database(Provider.<b>PostgreSQL</b>, host, dbName, userName, password, 30);
string mySQLVersion = db.GetString("select 'PostgreSQL version : ' || version()");@@
