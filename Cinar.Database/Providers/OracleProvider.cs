...
namespace Cinar.Database.Providers
{
    ...
    /// </summary>
    internal class OracleProvider : BaseProvider, IDatabaseProvider
    {
        public OracleProvider(IDatabase db, bool createDatabaseIfNotExist)
        {
            this.db = db;
            try
            {
                connection = new OracleConnection(db.ConnectionString);
            ...
            catch
            {
                if (createDatabaseIfNotExist)
                {
                    ...
                    connection = new OracleConnection(newConnStr);
                    try
                    {
                        connection.Open();
        ...
        {
            return new OracleConnection(db.ConnectionString);
        }
        ...
        {
            OracleCommand selectCommand = (OracleCommand)this.CreateCommand(selectCommandText, parameters);
        ...
        public IDbCommand CreateCommand(string cmdText, IDbTransaction transaction, params object[] parameters)
        {
            ...
            if (transaction != null)
                cmd = new OracleCommand(cmdText, (OracleConnection)this.Connection, (OracleTransaction)transaction);
            else
                cmd = new OracleCommand(cmdText, (OracleConnection)this.Connection);
